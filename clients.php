...
<?php
	...
	$sql2 = "SELECT Libelle, IDNiv2 FROM niv2 WHERE IDNiv1=2 AND IDNiv2=".$_GET['num']."  ORDER BY Ordre";
	...
	{	
		...
		while($resb=mysql_fetch_array($reqb))
		{
			...
			if($nbImagesClient!=0){
				echo "\t".'<div id="client_'.$a.'" class="client slidingSpaces panels client" title="'.utf8_decode($resb['Libelle']).'">'."\n";
				
				//Bouton de retour à l'accueil
				echo "\t\t".'<div id="menu_slide"><a href="./index.php" class="backToHome"><div class="bt_back_hor">Accueil <span style="color:#009cd5;">&nbsp;&gt;</span></div><div class="bt_back"><h1>Accueil</h1></div></a><div class="title_cat">'.ucfirst(utf8_decode($res2['Libelle'])).' ('.$c.'/'.$nbTrueClient.')</div></div>'."\n";
				...
				while($rescb2=mysql_fetch_array($reqcb2))
				{
					...
					if($k==0){
						...
						if(!preg_match("#$originalImagePath" . utf8_decode($resSqlImage['image'])."#i", $cache)){
							$f = fopen("cache.manifest", "a");
							fputs($f, $originalImagePath . utf8_decode($resSqlImage['image']) . "\n");
							fclose($f);
						}
					}
