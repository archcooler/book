...
(function(window, klass, Util){
	...
	PhotoSwipe.Cache.Functions = {
		 ...
		 * Default method for returning an image's description
		 * Assumes the el is an anchor and the first child is the
		 * image. The returned value is the "longdesc" attribute of the
		 * image.
		 */
		getImageDescription: function(el){
			
			if (el.nodeName === "IMG"){
				return Util.DOM.getAttribute(el, 'longdesc'); 
			}
			var i, j, childEl;
			for (i=0, j=el.childNodes.length; i<j; i++){
				childEl = el.childNodes[i];
				if (el.childNodes[i].nodeName === 'IMG'){
					return Util.DOM.getAttribute(childEl, 'longdesc'); 
				}
			}
...
(function(window, klass, Util){
	...
	PhotoSwipe.Toolbar.CssClasses = {
		...
		descriptionTop: 'ps-description-top',
...
(function(window, klass, Util){
	...
	PhotoSwipe.Toolbar.ToolbarClass = klass({
		...
		resetPosition: function(){
			...
			if (this.settings.target === window){
				if (this.settings.captionAndToolbarFlipPosition){
					...
					descriptionTop = Util.DOM.windowScrollTop() - Util.DOM.height(this.toolbarEl);
				}
				else {
					...
					captionTop = Util.DOM.windowScrollTop()+20;
				}	
				width = Util.DOM.windowWidth();
			}
			else{
				if (this.settings.captionAndToolbarFlipPosition){
					...
					descriptionTop = Util.DOM.height(this.settings.target) - Util.DOM.height(this.descriptionEl) - Util.DOM.height(this.toolbarEl);
					captionTop = 0;
				}
				width = Util.DOM.width(this.settings.target);
			}
		...
		fadeOut: function(){
			...
			Util.Animation.fadeOut(this.descriptionEl, this.settings.fadeOutSpeed, this.fadeOutHandler);
